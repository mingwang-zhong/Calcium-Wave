...
function SparkAnalysis()
    ...
    Tmax = 1500; % big enogh to contain a spark.
    ...
    % dlmwrite('spark_ci0.175_linescan.txt', [(1:Tmax).' spark_avg], 'delimiter', '\t');
    dlmwrite('I_release_ci0.175.txt', nonspark_I(1:total_nonspark,1), 'delimiter', '\t');

    data = dlmread('ci0.175/wholecell.txt');
    ...
    figure(1);
        ...
        saveas(gcf, 'SparkAnalysis_ci0.175.pdf', 'pdf');
    ...
    if total_nonspark > 0 
        figure(1);
            ...
            set(gcf, 'PaperPosition', [0.2 0 10 8]);
            set(gcf, 'PaperSize', [10 8]);
            saveas(gcf, 'SparkAnalysis_nonSpark_ci0.175.pdf', 'pdf');
...
function [spark_avg, nonspark_avg, total_spark, total_nonspark, nonspark_I] = linescan()
    ...
    data = dlmread('ci0.175/linescan.txt');
    num_CRU = 62;
    ...
    spark = zeros(Tmax, 30000, 10); % 1:I, 2:cjsr, 3:cp, 4:nou, 5:nob, 6:ncu, 7:ncb
    nonspark = zeros(Tmax, 30000, 8);
    nonspark_I = zeros(30000,2); % 1:peak, 2:duration
    ...
    dlmwrite('ci0.175_before_spark.txt', reshape(spark(10,1:(num_spark-1),2:10), [num_spark-1 9] ), 'delimiter', '\t');

    set(findobj('type','axes'),'FontSize',11);
    set(gcf, 'PaperPosition', [0.2 0 8 6]);
    set(gcf, 'PaperSize', [8 6]);
    saveas(gcf, 'before_Spark_ci0.175.pdf', 'pdf');
end