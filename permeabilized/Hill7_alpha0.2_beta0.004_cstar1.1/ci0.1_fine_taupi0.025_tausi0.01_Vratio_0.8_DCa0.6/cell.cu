...
/////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////
#define DT 	0.001 //ms, time step
#define stoptime (71.0) //ms
#define PCL 500.0	//ms, pacing cycle length
#define stopbeat 20
#define time_before_beat 100 //ms

#define out_step	1000 // number of steps to output data
...
#define Nx 	8	// Number of CRUs in the x direction
#define Ny 	8	// Number of CRUs in the y direction
#define Nz 	8	// Number of CRUs in the z direction
#define Nix	10	// number of lattices in x direction in a CRU
#define Niy	5	// number of lattices in y direction in a CRU
#define Niz	5	// number of lattices in z direction in a CRU
#define Nci	(Nix*Niy*Niz) // number of lattices in a CRU
...
#define Vi (0.953*0.8/Nci)	//um^3, Volume of the cytosolic space, for each compartment
...
#define taupi 0.025 //ms, Diffusion time from the proximal to the cytosol
#define tausi 0.01		//ms, Diffusion time from the submembrane to the cytosolic
...
#define taunl (4.2/25.0)		//ms, diffusion time of longitudinal NSR
#define taunt (1.3/6.25)		//ms, diffusion time of transverse NSR
#define tauil (2.269/25.0*0.5)	//ms, diffusion time of longitudinal cytosolic
#define tauit (0.963/6.25) 	//ms, diffusion time of transverse cytosolic
#define	Datp 	(5.8/2.7)	// diffusion time prefactor of ATP
#define Ddye 	(5.4/2.7)	// diffusion time prefactor of dye
...
#ifdef ISO
	#define Vup (200) // uM/ms, uptake strength
#else
	#define Vup (140) // uM/ms
#endif
...
#define corbularSR_percentage	0.0
...
#define RATIO 	1
#define ktson 	(0.00254*RATIO) // (uM*ms)^(-1), associate constant
...
#define ktfon 	(0.0327*RATIO)
#define ktfoff 	0.0196 // Troponin fast
#define Btf 	70.0

#define kcalon 	(0.0543*RATIO) // Calmodulin
...
#define konmyoca 	(0.0138*RATIO)
#define koffmyoca 	0.00046
#define Mgi 	1000.0 // Mg
#define Kmyomg 	(koffmyomg/konmyomg)
#define Kmyoca 	(koffmyoca/konmyoca)

#define katpon 	(0.15*RATIO) // ATP
#define katpoff 30.0
#define Batp 	727.0

#define kdyeon		(0.08*RATIO) // Dye
#define kdyeoff		0.09
#define Bdye		50.0

#define kegtaon 	(0.0015*RATIO) // EGTA
...
#define SX	(1.3/sqrt(8*log(2))) // sigma of the point spread function
#define SY	(0.4/sqrt(8*log(2)))
#define SZ	(0.4/sqrt(8*log(2)))
#define gx(i,ix,Ax) ( 1.0/sqrt(2*3.14159*SX*SX)*exp(-((i*Nix+ix)-(Ax))*((i*Nix+ix)-(Ax))*DX*DX/(2.0*SX*SX)) )
#define gy(j,jy,Ay)	( 1.0/sqrt(2*3.14159*SY*SY)*exp(-((j*Niy+jy)-(Ay))*((j*Niy+jy)-(Ay))*DY*DY/(2.0*SY*SY)) )
#define gz(k,kz,Az)	( 1.0/sqrt(2*3.14159*SZ*SZ)*exp(-((k*Niz+kz)-(Az))*((k*Niz+kz)-(Az))*DZ*DZ/(2.0*SZ*SZ))	)
			...
			for (k = 1; k < Nz-1; k++)
			{
				for (j = 1; j < Ny-1; j++)
				{
					...
					{	
						...
						Kat += h_CRU2[ps].Ka;
						cpt += h_CRU2[ps].cp;
						cjsrt += h_CRU2[ps].cjsr + h_CRU2[ps].cjsrc;
						cjt += h_CRU2[ps].Tcj;
				...
				if (step*DT>0000)
				{
					for (i =1; i < Nx-1; i++)
					{
						for (ix=0; ix<Nix;ix++)
						{
							int k = Nz/2, j = Ny/2;
							ps = pos(i,j,k);
							int crui = posi(ix,0,0);
							fprintf(linescan_file, 	"%g %g %g %g %g" "%g %g \n",
													
													step*DT,			(i*Nix+crui)*DX,
													h_CYT[ps*Nci+crui].ci, 	h_CRU2[ps].cp,
													h_CRU2[ps].cjsr, h_CBU[ps*Nci+crui].cadye,
													h_CRU2[ps].Jrel
			...
			///////////////////////////////////////// fluoxt ////////////////////////////////////
				...
				if (step*DT>0000)
				{
					...
					for (i=0;i<Nx;i++)
					{
						for ( ix=0;ix<Nix;ix++)
						{
							fprintf(fluoxt_file,"%g\t%g\t%g\n",
									
									step*DT,	(i*Nix+ix)*DX, 
									fluo[posall(i*Nix+ix, Ny/2*Niy+0, Nz/2*Niz+0)]
						...
						double f_Jmax, double alpha, double beta, int Hill, double custar, double cbstar)
{
	...
	if ((i*j*k)!=0 && i<Nx-1 && j<Ny-1 && k<Nz-1)
	{
		...
		CRU2[ps].Jrel = 0;//Nryr_Open * f_Jmax*0.00015 * (CRU2[ps].cjsr-CRU2[ps].cp)/Vp;
		
		if ( i==Nz/2 && j==Ny/2 && k==Nz/2 && step*DT>=10.0 )
		{
			double t = step*DT - 10;
			CRU2[ps].Jrel = 9.4*(1-exp(-t/6.409))*exp(-t/5.054)*100.0/2.0/9.6485/Vp; 
		}
		...
		if ( CRU2[ps].cjsrc > 0 )
		{
			...
			CRU2[ps].Jrelc = 0;//Nryr_Open_c * f_Jmax*0.00015 * (CRU2[ps].cjsrc-CYT[ps*Nci+1].ci)/(Vi/Nci);
		}
		else
			CRU2[ps].Jrelc = 0;
			...
			for ( int jy=0; jy<Niy; jy++ )
			{
				for ( int ix=0; ix<Nix; ix++ )
				{
					...
					double coupleATP = 	(CBU[knext].caatp-CBU[psi].caatp)/(Datp*tauit) +
										...
										(CBU[inext].caatp-CBU[psi].caatp)/(Datp*tauil) +
										(CBU[ibefore].caatp-CBU[psi].caatp)/(Datp*tauil);
					...
					double coupleegta = (CBU[knext].caegta-CBU[psi].caegta)/(Degta*tauit) +
										...
										(CBU[inext].caegta-CBU[psi].caegta)/(Degta*tauil) +
										(CBU[ibefore].caegta-CBU[psi].caegta)/(Degta*tauil);
					...
					CYT[psi].cinext = CYT[psi].ci 
									  +(	- CYT[psi].Juptake
											...
											+( (crui==posi(0,0,1))?( (Vs/Vi)*(diffsi1+diffsiatp1)+(Vp/Vi)*(diffpi1+diffpiatp1) ):0 ) 
					...
					CYT[psi].cnsrnext = CYT[psi].cnsr 
										+(  CYT[psi].Juptake * Vi/Vnsr
											...
											+( (crui==posi(0,0,1))?( diffjn1*Vjsr/Vnsr ):0 	)
					...
					CBU[psi].cadyenext += ( buffdye + coupledye
											...
											+( (crui==posi(0,0,1))?( (Vs/Vi)*(diffsidye1)+(Vp/Vi)*(diffpidye1) ):0 ) )*DT;
					CBU[psi].caegtanext += ( buffegta + coupleegta
											+( (crui==0)?( (Vs/Vi)*(diffsiegta0)+(Vp/Vi)*(diffpiegta0) ):0 )
											+( (crui==posi(0,0,1))?( (Vs/Vi)*(diffsiegta1)+(Vp/Vi)*(diffpiegta1) ):0 ) )*DT;
		...
		{
			////////////////////// submembrane: dotcs ///////////////////////// 
			double csdiff = ( CRU[pos(i,j+1,k)].cs+CRU[pos(i,j-1,k)].cs-2*CRU[ps].cs )/taust
							+ ( CRU[pos(i,j,k+1)].cs+CRU[pos(i,j,k-1)].cs-2*CRU[ps].cs )/taust ;
			double csdiffatp = ( SBU[pos(i,j+1,k)].caatp+SBU[pos(i,j-1,k)].caatp-2*SBU[ps].caatp )/(Datp*taust)
							 + ( SBU[pos(i,j,k+1)].caatp+SBU[pos(i,j,k-1)].caatp-2*SBU[ps].caatp )/(Datp*taust);
			double csdiffdye = ( SBU[pos(i,j+1,k)].cadye+SBU[pos(i,j-1,k)].cadye-2*SBU[ps].cadye )/(Ddye*taust)
							 + ( SBU[pos(i,j,k+1)].cadye+SBU[pos(i,j,k-1)].cadye-2*SBU[ps].cadye )/(Ddye*taust);
			double csdiffegta = ( SBU[pos(i,j+1,k)].caegta+SBU[pos(i,j-1,k)].caegta-2*SBU[ps].caegta )/(Degta*taust)
