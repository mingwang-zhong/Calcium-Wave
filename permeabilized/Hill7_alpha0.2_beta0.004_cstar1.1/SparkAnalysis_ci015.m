...
function SparkAnalysis()
    ...
    Tmax = 800; % big enogh to contain a spark.
    ...
    [N2,edge2]=histcounts(spark_profile(:,2)); % FWHM
    ...
    dlmwrite('spark_ci0.15_FF0.txt', [center1', N1'], 'delimiter', '\t');
    dlmwrite('spark_ci0.15_FWHM.txt', [center2', N2'], 'delimiter', '\t');
    dlmwrite('spark_ci0.15_FDHM.txt', [center3', N3'], 'delimiter', '\t');

    sparkrate = total_spark/(total_time-Tmax)*1000;
    nonsparkrate = total_nonspark/(total_time-Tmax)*1000;
    
    dlmwrite('spark_ci0.15.txt', [(1:Tmax).' spark_avg], 'delimiter', '\t');

    data = dlmread('ci0.15/wholecell.txt');
    ...
    dlmwrite('spark_statistics_0.15.txt', temp, 'delimiter', '\t');
    ...
    figure(1);
        ...
        saveas(gcf, 'SparkAnalysis_ci0.15.pdf', 'pdf');
    ...
    if total_nonspark > 0 
        figure(1);
            ...
            set(gcf, 'PaperPosition', [0.2 0 10 8]);
            set(gcf, 'PaperSize', [10 8]);
            saveas(gcf, 'SparkAnalysis_nonSpark_ci0.15.pdf', 'pdf');
...
function [spark_avg, nonspark_avg, total_spark, total_nonspark, nonspark_I] = linescan()
    ...
    data = dlmread('ci0.15/linescan.txt');
    num_CRU = 62;
    linescan = zeros(num_CRU, 18, total_time);
    for t=1:total_time
        linescan(:,:,t) = data( ( (t-1)*num_CRU+1 ):( t*num_CRU ), :);
    end
    
    % Tmax:maximum open time, 5000: maximum number of releases
    spark = zeros(Tmax, 5000, 8); % 1:I, 2:cjsr, 3:cp, 4:nou, 5:nob, 6:ncu, 7:ncb
...
function [spark_profile, spark_location] = SparkProfile()
    ...
    FluoSizeX = 128;
    fluo_basal = 6;
    fluo_threshold = fluo_basal*1.6;

    fluoxt = dlmread('ci0.15/fluoxt.txt');
    fluo = zeros(total_time+Tmax*2, FluoSizeX+Xmax, 5); %It is larger because of boudary.
    for t = 1:total_time
    ...
    total_spark = num_spark - 1;
    dlmwrite('spark_ci0.15_time.txt', [(1:(Tmax+1)).' spark_time_trace], 'delimiter', '\t');
end
