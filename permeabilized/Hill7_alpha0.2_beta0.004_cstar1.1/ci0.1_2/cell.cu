					...
					for (i =1; i < Nx-1; i++)
                                        {
                                                ...
                                                {
                                                        fprintf(linescan_file,  "%g %g %g %g %g %g\n",

                                                                                                        step*DT,                        (double)(i*Nix+ii)*DX,
                                                                                                        h_CYT[ps*Nci+posi(ii,0,0)].ci, h_CYT[ps*Nci+posi(ii,0,1)].ci,
                                                                                                        h_CBU[ps*Nci+posi(ii,0,0)].cadye, h_CBU[ps*Nci+posi(ii,0,1)].cadye
			...
			///////////////////////////////////////// fluoxt ////////////////////////////////////
				...
				if (step*DT>2000)
				{
					...
					for (i=0;i<Nx;i++)
					{
						...
						for ( ix=0;ix<Nix;ix++)
						{
							fprintf(fluoxt_file,"%g %g %g %g %g" " %g %g\n",
									
									step*DT,	(i*Nix+ix)*DX,
									fluo[posall(i*Nix+ix, Ny/2*Niy+0, 4*Niz+0)], fluo[posall(i*Nix+ix,(Ny/2-3)*Niy+0,6*Niz+0)], 
									fluo[posall(i*Nix+ix,(Ny/2+3)*Niy+0,6*Niz+0)], h_CBU[ps*Nci+posi(ix,0,0)].cadye,
									h_CBU[ps*Nci+posi(ix,0,0)].caatp
