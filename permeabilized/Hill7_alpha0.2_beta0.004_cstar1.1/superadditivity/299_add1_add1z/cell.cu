...
#define tauil 2.269	//ms, diffusion time of longitudinal cytosolic
	...
	while ( step*DT < stoptime )
	{
		...
		//	matrix2file(h_CYT, step);
			...
			fprintf(wholecell_file,	"%g %g %g %g %g " "%g %g %g %g %g "
									...
									nlcc_act, nlcc_v,
									nlcc_Ca, spark_rate,
									h_CRU2[pos(Nx/2,Ny/2,Nz/2)].cp, h_CRU2[pos(Nx/2+1,Ny/2,Nz/2)].cp,
									h_CRU2[pos(Nx/2,Ny/2,Nz/2)].Jrel, h_CRU2[pos(Nx/2,Ny/2,Nz/2-4)].Jrel,

									h_CRU2[pos(Nx/2,Ny/2,Nz/2-3)].Jrel, h_CRU2[pos(Nx/2,Ny/2,Nz/2-2)].Jrel,
									h_CRU2[pos(Nx/2,Ny/2,Nz/2-1)].Jrel, h_CRU2[pos(Nx/2,Ny/2,Nz/2+1)].Jrel,
									h_CRU2[pos(Nx/2,Ny/2,Nz/2+2)].Jrel, h_CRU2[pos(Nx/2,Ny/2,Nz/2+3)].Jrel,
									h_CRU2[pos(Nx/2,Ny/2,Nz/2+4)].Jrel
					);
						...
						double f_Jmax, double alpha, double beta, int Hill, double custar, double cbstar)
{
	...
	if ((i*j*k)!=0 && i<Nx-1 && j<Ny-1 && k<Nz-1)
	{
		...
		if (ts>=0 && ts<=40)
		{
			...
			double p7 =1.279;
			double p8 =-0.01576;
			Irel = p1*pow(ts,7) + p2*pow(ts,6) + p3*pow(ts,5) + p4*pow(ts,4) + p5*pow(ts,3) + p6*ts*ts + p7*ts + p8;
		}
		else if (ts>40 && ts<=100)
			Irel = -0.0016*ts+0.16;
		else 
			Irel = 0;
		...
		CRU2[ps].Jrel = 0;
		if ( ( (i==Nx/2 || i==Nx/2-1) && (j>=Ny/2-4 && j<=Ny/2+4) && (k>=Nz/2-4 && k<=Nz/2+4) ) 
				|| (i==Nx/2+1 && j>=Ny/2+1 && k>=Nz/2 ) 
				|| (i==Nx/2+1 && j==Ny/2 && k==Nz/2+1) )
			CRU2[ps].Jrel = Irel*100.0/2.0/9.6485/Vp; 

		if (i==Nx/2 && j==Ny/2 && k==Nz/2 && step%50==0)
			printf("t = %g\tIrel = %g\n", step*DT, Irel);
		...
		for ( int kz=0; kz<Niz; kz++ )
		{
			for ( int jy=0; jy<Niy; jy++ )
			{
				for ( int ix=0; ix<Nix; ix++ )
				{
					...
					CBU[psi].caatpnext = Batp*CYT[psi].cinext/(katpoff/katpon+CYT[psi].cinext);
