...
int main(int argc, char **argv)
{
	...
	/////////////////////////////////// variables /////////////////////////////////////////////////
	int step = 0;
	int i, j, k, ix, jy; // i,j,k for CRU index; ix, jy, kz for lattices in each CRU
	...
	FILE * wholecell_file = fopen("wholecell.txt","w");
		...
		FILE * linescan_file_y = fopen("linescan_y.txt","w");
	#endif

	#ifdef output_fluoxt
		FILE * fluoxt_file = fopen("fluoxt.txt","w");
		FILE * fluoxt_file_y = fopen("fluoyt.txt","w");
	#endif
	...
	while ( step*DT < stoptime )
	{
		...
		//	matrix2file(h_CYT, step);
			...
			////////////////////////////// Line Scan ////////////////////////////////////	flag
				...
				if ( step*DT>000 )
				{
					for (i =1; i < Nx-1; i++)
					{
						int k = 4, j = Ny/2;
						ps = pos(i,j,k);
						fprintf(linescan_file, 	"%g %g %g %g %g " "%g %g %g %g %i "
												"%g %g %g %i %i " "%i %i %g %g %g "
												"%i %i %i %i %g " "%g %g %i %i %i "
												"%i\n",
												
												step*DT,			(double)i,
												h_CYT[ps*Nci].ci, 	h_CRU[ps].cs, 		
												h_CYT[ps*Nci].cnsr,	h_CYT[ps*Nci].Juptake,	
												h_CRU2[ps].Jleak,	h_CRU[ps].JCa, 
												h_CRU[ps].JNCX,		h_CRU2[ps].nLCC_open,  

												h_CRU2[ps].cp,		h_CRU2[ps].cjsr,
												h_CRU2[ps].Jrel,	h_CRU2[ps].nou, 	
												h_CRU2[ps].nob,		h_CRU2[ps].ncu, 	
												h_CRU2[ps].ncb, 	h_CRU2[pos(i,Ny/2+5,7)].cp, 
												h_CRU2[pos(i,Ny/2+5,7)].cjsr, h_CRU2[pos(i,Ny/2+5,7)].Jrel, 

												h_CRU2[pos(i,Ny/2+5,7)].nou, 	h_CRU2[pos(i,Ny/2+5,7)].nob,		
												h_CRU2[pos(i,Ny/2+5,7)].ncu, 	h_CRU2[pos(i,Ny/2+5,7)].ncb,
												h_CRU2[pos(i,Ny/2-5,7)].cp, 	h_CRU2[pos(i,Ny/2-5,7)].cjsr, 
												h_CRU2[pos(i,Ny/2-5,7)].Jrel, 	h_CRU2[pos(i,Ny/2-5,7)].nou, 	
												h_CRU2[pos(i,Ny/2-5,7)].nob,	h_CRU2[pos(i,Ny/2-5,7)].ncu, 

												h_CRU2[pos(i,Ny/2-5,7)].ncb
					...
					for (j =1; j < Ny-1; j++)
					{
						int k = 4, i = 50;
						ps = pos(i,j,k);
						fprintf(linescan_file_y,"%g %g %g %g %g " "%g %g %g %g %i "
												"%g %g %g %i %i " "%i %i %g %g %g \n",
												...
												h_CRU2[ps].Jrel,	h_CRU2[ps].nou, 	
												h_CRU2[ps].nob,		h_CRU2[ps].ncu, 	
												h_CRU2[ps].ncb,     h_CYT[pos(10,j,k)].ci,
												h_CYT[pos(25,j,k)].ci, h_CYT[pos(40,j,k)].ci
											 
							);
					}
					fprintf(linescan_file_y, "\n");
					fflush(linescan_file_y);
				}
			#endif
			...
			///////////////////////////////////////// fluoxt ////////////////////////////////////
				...
				if (step*DT>000)
				{
					...
					for (i=0;i<Nx;i++)
					{
						for ( ix=0;ix<Nix;ix++)
						{
							fprintf(fluoxt_file,"%g\t%g\t%g\t%g\t%g\n",
									...
									fluo[posall(i*Nix+ix,(Ny/2+3)*Niy+0,6*Niz+0)]
					...
					for (j=0;j<Ny;j++)
					{
						for ( jy=0;jy<Niy;jy++)
						{
							fprintf(fluoxt_file_y,"%g\t%g\t%g\t%g\t%g\n",
									
									step*DT,	(j*Niy+jy)*DY,
									fluo[posall(50*Nix+0, j*Niy+jy, 4*Niz+0)], fluo[posall(10*Nix+0,j*Niy+jy,4*Niz+0)], 
									fluo[posall(25*Nix+0, j*Niy+jy, 4*Niz+0)]
									
									);
						}
					}
				
					fprintf(fluoxt_file_y,"\n");
					fflush( fluoxt_file_y );
				}
			#endif

			/////////////////////////////////////////////////////////////////////////////
			// if (step%(out_step*10)==0 || ( step*DT<15001 ) )
			// {
			// 	char FileName[50];
				// sprintf(FileName,"image/%-s%d%s","ci_xy_",(int)(step*DT),"ms.txt");
				// FILE * ci_file = fopen(FileName,"w");
				...
				// 				for ( ix=0;ix<Nix;ix++)
				// 				{
				// 					fprintf(ci_file,"%g\t", h_CYT[pos(i,j,Nz/2)*Nci+posi(ix,iy,0)].ci);
				// 				}
				// 			}
				// 			fprintf(ci_file,"\n");
				// 		}
				...
				// sprintf(FileName,"image/%-s%d%s","ci_xz_",(int)(step*DT),"ms.txt");
				// ci_file = fopen(FileName,"w");
				...
				// 				for ( ix=0;ix<Nix;ix++)
				// 				{
				// 					fprintf(ci_file,"%g\t", h_CYT[pos(i,9,k)*Nci+posi(ix,0,kz)].ci);
				// 				}
				// 			}
				// 			fprintf(ci_file,"\n");
				// 		}
			...
			if ( ( step%(out_step*10)==0 && step*DT>24000 ) || ( step*DT>24650 && step*DT<24850 ) )
			{
				char FileName[50];
				sprintf(FileName,"image_cp/%-s%d%s","cp_xy_",(int)(step*DT),"ms.txt");
				FILE * ci_file = fopen(FileName,"w");
				...
				if (step*DT>0000)
				{
					for (j=1;j<Ny-1;j++)
					{
						for (i=1;i<Nx-1;i++)
						{
							fprintf(ci_file,"%g\t", h_CRU2[pos(i,j,Nz/2)].cp);
						}
						fprintf(ci_file,"\n");
					}
				}
				...
				sprintf(FileName,"image_cp/%-s%d%s","cp_xz_",(int)(step*DT),"ms.txt");
				ci_file = fopen(FileName,"w");
				...
				if (step*DT>0000)
				{
					for (k=1;k<Nz-1;k++)
					{
						for (i=1;i<Nx-1;i++)
						{
							fprintf(ci_file,"%g\t", h_CRU2[pos(i,19,k)].cp);
						}
						fprintf(ci_file,"\n");
					}
				}
				...
				sprintf(FileName,"image_cp/%-s%d%s","cp_xz_",(int)(step*DT),"ms_20.txt");
				ci_file = fopen(FileName,"w");
				...
				if (step*DT>0000)
				{
					for (k=1;k<Nz-1;k++)
					{
						for (i=1;i<Nx-1;i++)
						{
							fprintf(ci_file,"%g\t", h_CRU2[pos(i,20,k)].cp);
						}
						fprintf(ci_file,"\n");
					}
				}
				...
				sprintf(FileName,"image_cp/%-s%d%s","cp_xz_",(int)(step*DT),"ms_21.txt");
				ci_file = fopen(FileName,"w");
				...
				if (step*DT>0000)
				{
					for (k=1;k<Nz-1;k++)
					{
						for (i=1;i<Nx-1;i++)
						{
							fprintf(ci_file,"%g\t", h_CRU2[pos(i,21,k)].cp);
						}
						fprintf(ci_file,"\n");
					}
				}
				...
				sprintf(FileName,"image_cp/%-s%d%s","cp_xz_",(int)(step*DT),"ms_22.txt");
				ci_file = fopen(FileName,"w");
				...
				if (step*DT>0000)
				{
					for (k=1;k<Nz-1;k++)
					{
						for (i=1;i<Nx-1;i++)
						{
							fprintf(ci_file,"%g\t", h_CRU2[pos(i,22,k)].cp);
						}
						fprintf(ci_file,"\n");
					}
				}
			...
			if (step%(out_step*10)==0 )
			{
				char FileName[50];
				sprintf(FileName,"video/%-s%d%s","ci",(int)(step*DT),"ms.vtk");
				FILE * ci_file = fopen(FileName,"w");

				fprintf(ci_file, "# vtk DataFile Version 3.0\n");
				fprintf(ci_file, "3d\n");
				fprintf(ci_file, "ASCII\n");
				fprintf(ci_file, "DATASET STRUCTURED_POINTS\n");
				fprintf(ci_file, "DIMENSIONS %d %d %d\n",(Nx-2)*Nix,(Ny-2)*Niy,(Nz-2)*Niz);
				fprintf(ci_file, "ASPECT_RATIO 1 1 1\n");
				fprintf(ci_file, "ORIGIN 0 0 0\n");
				fprintf(ci_file, "POINT_DATA %d\n",(Nx-2)*Nix*(Ny-2)*Niy*(Nz-2)*Niz);
				fprintf(ci_file, "SCALARS ci double 1\n");
				fprintf(ci_file, "LOOKUP_TABLE default\n\n");
				int jy,kz;
				for (k=1;k<(Nz-1);k++)
				{
					for (kz=0;kz<Niz;kz++)
						...
						for (j=1;j<(Ny-1);j++)
						{
							for (jy=0;jy<Niy;jy++)
							{
								for (i=1;i<(Nx-1);i++)
								{
									for (ix=0;ix<Nix;ix++)
									{
										fprintf(ci_file,"%g \t", h_CYT[pos(i,j,k)*Nci+posi(ix,jy,kz)].ci);
									}
								}
								fprintf(ci_file, "\n");
							}
	...
	fclose(wholecell_file);
		...
		fclose(linescan_file_y);
	#endif
	
	#ifdef output_fluoxt
		fclose(fluoxt_file);
		fclose(fluoxt_file_y);
	#endif
