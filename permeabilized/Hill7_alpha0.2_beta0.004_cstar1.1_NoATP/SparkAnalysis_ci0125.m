...
function SparkAnalysis()
    ...
    dlmwrite('spark_ci0.125_FF0.txt', [center1', N1'], 'delimiter', '\t');
    dlmwrite('spark_ci0.125_FWHM.txt', [center2', N2'], 'delimiter', '\t');
    dlmwrite('spark_ci0.125_FDHM.txt', [center3', N3'], 'delimiter', '\t');

    sparkrate = total_spark/(total_time-Tmax)*1000;
    nonsparkrate = total_nonspark/(total_time-Tmax)*1000;
    
    dlmwrite('spark_ci0.125.txt', [(1:Tmax).' spark_avg], 'delimiter', '\t');

    data = dlmread('ci0.125/wholecell.txt');
    ...
    dlmwrite('spark_statistics_0.125.txt', temp, 'delimiter', '\t');
    ...
    figure(1);
        ...
        saveas(gcf, 'SparkAnalysis_ci0.125.pdf', 'pdf');
    ...
    if total_nonspark > 0 
        figure(1);
            ...
            set(gcf, 'PaperPosition', [0.2 0 10 8]);
            set(gcf, 'PaperSize', [10 8]);
            saveas(gcf, 'SparkAnalysis_nonSpark_ci0.125.pdf', 'pdf');
...
function [spark_avg, nonspark_avg, total_spark, total_nonspark, nonspark_I] = linescan()
    ...
    data = dlmread('ci0.125/linescan.txt');
    num_CRU = 62;
...
function [spark_profile, spark_location] = SparkProfile()
    ...
    FluoSizeX = 128;
    fluo_basal = 5.2;
    fluo_threshold = fluo_basal*1.65;

    fluoxt = dlmread('ci0.125/fluoxt.txt');
    ...
    total_spark = num_spark - 1;
    dlmwrite('spark_ci0.125_time.txt', [(1:(Tmax+1)).' spark_time_trace], 'delimiter', '\t');
end
