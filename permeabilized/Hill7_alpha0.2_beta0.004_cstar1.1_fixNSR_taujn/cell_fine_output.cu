...
#define stoptime (atof(argv[10])) // (200001.0) //ms
#define PCL 2000.0	//ms, pacing cycle length
#define stopbeat 18
#define time_before_beat 100 //ms

#define out_step	1 // number of steps to output data
...
#define tautr (20.0)	//ms, Diffusion time from NSR to JSR 
...
#define	Vleak (0.0) // ms^{-1}, Shannon et al 2004, Eq. 107 (0.00212 = 5.348e-6*0.5/0.00126 )
...
__global__ void Initial( cru *CRU, cru2 *CRU2, cytosol *CYT, cyt_bu *CBU, sl_bu *SBU, double ci_b, double ci_internal, double cj_b);
__global__ void Compute( cru *CRU, cru2 *CRU2, cytosol *CYT, cyt_bu *CBU, sl_bu *SBU, double v, int step, double nai, 
						double f_Jmax, double alpha, double beta, int Hill, double custar, double cbstar, double cj_b);
__global__ void Finish( cru *CRU, cru2 *CRU2, cytosol *CYT, cyt_bu *CBU, sl_bu *SBU, int step );
...
int main(int argc, char **argv)
{
	...
	double ci_internal = atof(argv[11]);
	...
	click = (int*) malloc(Nx*Ny*Nz*sizeof(int));
	...
	FILE * time_to_spark_file = fopen("time_to_1st_spark.txt","w");
	...
	Initial<<<numBlocks, threadsPerBlock>>>(d_CRU, d_CRU2, d_CYT, d_CBU, d_SBU, ci_basal, ci_internal, cjsr_basal);
	...
	while ( step*DT < stoptime )
	{
		...
		//	matrix2file(h_CYT, step);
			...
			for (k=1;k<Nz-1;k++)
			{
				for (j=1;j<Ny-1;j++)
				{
					for (i=1;i<Nx-1;i++)
					{
						if (h_CRU2[pos(i,j,k)].Jrel>1 && click[pos(i,j,k)]==0)
						{
							fprintf(time_to_spark_file, "%g %g %g %d %d " "%d %d %d %d %d\n", 
								
								step*DT, h_CRU2[pos(i,j,k)].cp_before, 
								h_CRU2[pos(i,j,k)].cjsr,  h_CRU2[ps].nou, 
			...
			////////////////////////////// Line Scan ////////////////////////////////////	flag
				...
				if (1) // ( step*DT>000 )
				{
					for (i =1; i < Nx-1; i++)
					{
						fprintf(linescan_file, 	"%g %d %g %g %g " "%g %g %g %g %g "
												"%g %g %g %g \n",
												
												step*DT,			i,
												h_CRU2[pos(i,3,3)].Jrel,   h_CRU2[pos(i,13,3)].Jrel,
												h_CRU2[pos(i,18,3)].Jrel,  h_CRU2[pos(i,23,3)].Jrel,
												// h_CRU2[pos(i,3,8)].Jrel,   h_CRU2[pos(i,13,8)].Jrel, 
												// h_CRU2[pos(i,18,8)].Jrel,  h_CRU2[pos(i,23,8)].Jrel,

												h_CRU2[pos(i,3,3)].cp,   h_CRU2[pos(i,13,3)].cp,
												h_CRU2[pos(i,18,3)].cp,  h_CRU2[pos(i,23,3)].cp,
												// h_CRU2[pos(i,3,8)].cp,   h_CRU2[pos(i,13,8)].cp, 
												// h_CRU2[pos(i,18,8)].cp,  h_CRU2[pos(i,23,8)].cp,

												h_CRU2[pos(i,3,3)].cjsr,   h_CRU2[pos(i,13,3)].cjsr,
												h_CRU2[pos(i,18,3)].cjsr,  h_CRU2[pos(i,23,3)].cjsr
												// h_CRU2[pos(i,3,8)].cjsr,   h_CRU2[pos(i,13,8)].cjsr, 
												// h_CRU2[pos(i,18,8)].cjsr,  h_CRU2[pos(i,23,8)].cjsr
		...
		Compute<<<numBlocks, threadsPerBlock>>>( d_CRU, d_CRU2, d_CYT, d_CBU, d_SBU, v, step, nai, f_Jmax, alpha, beta, Hill, custar, cbstar, cjsr_basal);
		Finish<<<numBlocks, threadsPerBlock>>>( d_CRU, d_CRU2, d_CYT, d_CBU, d_SBU, step);
		step++;
	}

	fclose(wholecell_file);
	fclose(time_to_spark_file);
...
__global__ void Initial( cru *CRU, cru2 *CRU2, cytosol *CYT, cyt_bu *CBU, sl_bu *SBU, double ci_bb, double ci_internal, double cj_b)
{
