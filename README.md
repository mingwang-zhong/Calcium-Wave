...
The codes that produce propagating Ca waves in permeabilized and intact myocytes are provided below

```bash
permeabilized/Hill7_alpha0.2_beta0.004_cstar1.1/ci0.3/cell.cu
intact/Hill7_cstar1.1_ISO/cell.cu
```

These two codes generate `.vtk` files used for generating two videos in the paper. The visualization application [ParaView](http://paraview.org) can open these `.vtk` files and produce videos.
