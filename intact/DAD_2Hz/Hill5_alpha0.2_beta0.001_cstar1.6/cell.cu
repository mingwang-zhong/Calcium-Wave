...
int main(int argc, char **argv)
{
	...
	FILE * wholecell_file2 = fopen("wholecell2.txt","w");
	...
	while ( step*DT < stoptime )
	{
		...
		//	matrix2file(h_CYT, step);
			...
			fprintf(wholecell_file2, "%g %g\n", step*DT, I_Na);
			fflush( wholecell_file2 );


			////////////////////////////// Line Scan ////////////////////////////////////	flag
			#ifdef output_linescan
				// if ( step*DT > ( stopbeat - 2 )*PCL && step*DT < ( stopbeat + 2 )*PCL )
				if ( step*DT>000 && step*DT<15000 && step%(out_step)==0 )
				{
					for (i =1; i < Nx-1; i++)
					{
						int k = 4, j = Ny/2;
