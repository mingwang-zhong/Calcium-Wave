...
int main(int argc, char **argv)
{
	...
	FILE * wholecell_file = fopen("wholecell.txt","w");
		...
		FILE * linescan_file2 = fopen("linescan_transverse.txt","w");
	#endif
	...
	while ( step*DT < stoptime )
	{
		...
		//	matrix2file(h_CYT, step);
			...
			////////////////////////////// Line Scan ////////////////////////////////////	flag
				...
				if ( step*DT>000 && step*DT<25000 )
				{
					...
					for (j =1; j < Ny-1; j++)
					{
						int k = 4, i = 20;
						ps = pos(i,j,k);
						fprintf(linescan_file2, "%g %g %g %g %g " "%g %g %g %g %g "
												"%g %g %i %i %i " "%i %i %g \n",
					...
					fprintf(linescan_file2, "\n");
					fflush(linescan_file2);
				}
			#endif
			...
			///////////////////////////////////////// video //////////////////////////////////// flag
			// if ( step%(10*out_step)==0 && step*DT>=000 && step*DT<=25000 )
			// 	matrix2file(h_CYT, step);
