...
// #ifdef ISO
// 	#define Vup (320.0) // uM/ms, uptake strength
// #else
...
__global__ void Initial( cru *CRU, cru2 *CRU2, cytosol *CYT, cyt_bu *CBU, sl_bu *SBU, double ci_b, double cj_b, double corbularSR_percentage);
__global__ void Compute( cru *CRU, cru2 *CRU2, cytosol *CYT, cyt_bu *CBU, sl_bu *SBU, double v, int step, double nai, 
						double f_Jmax, double alpha, double beta, int Hill, double custar, double cbstar, double Vup);
...
__device__ double uptake(double ci, double cnsr, double Vup);
...
int main(int argc, char **argv)
{
	...
	double corbularSR_percentage = atof(argv[12]);
	setup_kernel<<<numBlocks, threadsPerBlock>>>(atoi(argv[10]),d_CRU2);
	Initial<<<numBlocks, threadsPerBlock>>>(d_CRU, d_CRU2, d_CYT, d_CBU, d_SBU, ci_basal, cjsr_basal, corbularSR_percentage);
	...
	while ( step*DT < stoptime )
	{
		...
		double Vup = atof(argv[11]);

		Compute<<<numBlocks, threadsPerBlock>>>( d_CRU, d_CRU2, d_CYT, d_CBU, d_SBU, v, step, nai, f_Jmax, alpha, beta, Hill, custar, cbstar, Vup);
		...
		for ( int kz=0; kz<Niz; kz++ )
		{
			for ( int jy=0; jy<Niy; jy++ )
			{
				for ( int ix=0; ix<Nix; ix++ )
				{
					...
					CYT[psi].Juptake = uptake(CYT[psi].ci, CYT[psi].cnsr, Vup);
...
__device__ double uptake1(double ci, double cnsr)		//uptake
{
	...
	double Knsr = 1700.0;
	double HH = 1.787;
	double Iuptake = (pow(ci/Ki,HH)-pow(cnsr/Knsr,HH)) / (1.0+pow(ci/Ki,HH)+pow(cnsr/Knsr,HH));
	return Iuptake;
}


__device__ double uptake(double ci, double cnsr, double Vup)			//uptake
{
