...
for zz=16:25
    for yy=1:62
        ...
        [cjsr_nadir,t_nadir] = findpeaks(cjsmooth,...
            'MinPeakDistance',40,'MinPeakHeight',-NadirThreshold);
        ...
        for i=1:length(t_nadir)
            ...
            if i>1 && t_nadir(i-1)>Time_Stable
                ...
                if fall1st > SparkThreshold %&& t_nadir(i)-t_nadir(i-1)>=5%(cc,kk)
...
fo = fitoptions('Method','NonlinearLeastSquares',...
                   'Lower',[0,0,0],...
                   'Upper',[400,1000,1000],...
                   'StartPoint',[200,70,150]);
    ft = fittype('a-a*exp(-(x-b)/c)','options',fo);
    [par,~] = fit(restitution(3:end,1),restitution(3:end,2),ft);
