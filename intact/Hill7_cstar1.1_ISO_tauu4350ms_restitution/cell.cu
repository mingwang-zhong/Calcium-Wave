				...
				// if ( step*DT>12000 && step*DT<25000 )
				{
					for (i =1; i < Nx-1; i++)
					{
						...
						fprintf(linescan_file, 	"%g %g %g %g %g " "%g %g %g %g %g "
												...
												h_CRU[ps].cs, 		h_CRU2[ps].Jrel,
												h_CRU2[pos(i,j,k-1)].Jrel, h_CRU2[pos(i,j,k+1)].Jrel,
												h_CRU2[pos(i,j-1,k)].Jrel, h_CRU2[pos(i,j+1,k)].Jrel,

												h_CRU2[pos(i,j-1,k-1)].Jrel, h_CRU2[pos(i,j-1,k+1)].Jrel,
												h_CRU2[pos(i,j+1,k-1)].Jrel, h_CRU2[pos(i,j+1,k+1)].Jrel,
												h_CRU2[pos(i,j+2,k+2)].Jrel, h_CRU2[ps].cjsr,
												h_CRU2[pos(i,j,k-1)].cjsr, h_CRU2[pos(i,j,k+1)].cjsr,
												h_CRU2[pos(i,j-1,k)].cjsr, h_CRU2[pos(i,j+1,k)].cjsr,
												
												h_CRU2[pos(i,j-1,k-1)].cjsr, h_CRU2[pos(i,j-1,k+1)].cjsr,
												h_CRU2[pos(i,j+1,k-1)].cjsr, h_CRU2[pos(i,j+1,k+1)].cjsr,
												h_CRU2[pos(i,j+2,k+2)].cjsr
...
__global__ void Initial( cru *CRU, cru2 *CRU2, cytosol *CYT, cyt_bu *CBU, sl_bu *SBU, double ci_b, double cj_b)
{
	...
	double kb2u = 1.0/4350.0;//(  4000.0/( 1.0+pow(CRU2[ps].cjsr/670.0, 24) ) + 350.0  );
							...
							double f_Jmax, double alpha, double beta, int Hill, double custar, double cbstar)
{
	...
	double ku2b = 1.0/( 1.0+pow(cb/BCSQN/13.3, 24) )/taub;
	double kb2u = 1.0/4350.0; //(  4000.0/( 1.0+pow(cjsr/670.0, 24) ) + 350.0  );
