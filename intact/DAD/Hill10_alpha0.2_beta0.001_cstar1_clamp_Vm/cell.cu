...
int main(int argc, char **argv)
{
	...
	#ifdef APclamp
		FILE *vm_file = fopen("../Vm_Hill10_alpha0.2_beta0.001_cstar1_clamp_cp.txt","r");
		double ap[590][2] = {0.0};
		for (i=0; i<590; i++)
		{
			fscanf(vm_file, "%lf %lf\n", &ap[i][0], &ap[i][1]);
		}
		fclose(vm_file);
	#endif
	...
	while ( step*DT < stoptime )
	{
		...
		#ifdef APclamp
			double t0 = 14208.0;
			if (step*DT>t0 && step*DT<t0+590)
			{
				i = (int) (step*DT - t0);
				v = ap[i][1] + ( step*DT - (int)(step*DT) )*( ap[i+1][1] - ap[i][1] );
			}
		#endif
		...
		//	matrix2file(h_CYT, step);
			...
			////////////////////////////// output to screen /////////////////////////////
			if ( step%(10*out_step)==0 )
			{
				end_time=clock()/(1.0*CLOCKS_PER_SEC);	
				printf(	"t=%g\t/ %g\t\ttime = %.1fs = %.1fh\t\tcit = %g\t\tcjsrt = %g\t\tVm = %g\n",
						step*DT, stoptime, 
			...
			////////////////////////////// Line Scan ////////////////////////////////////	flag
				...
				if ( step*DT>14000 && step*DT<15000 )
				{
					for (i =1; i < Nx-1; i++)
					{
						int k = 4, j = Ny/2;
