...
Time_Stable=4000;
NadirThreshold = 662;
SparkThreshold = 200; % assume spark size is related to SR load
...
for i=1:max(idx)
    restitution(i,2) = mean(cjsr_ratio(idx==i,2)); % ratio
    restitution(i,3) = std(cjsr_ratio(idx==i,2)); % error
end
...
fo = fitoptions('Method','NonlinearLeastSquares',...
                   'Lower',[0,10,0],...
                   'Upper',[2,300,1000],...
                   'StartPoint',[1,100,150]);
    ft = fittype('a-a*exp(-(x-b)/c)','options',fo);
    [par,~] = fit(restitution(7:50,1),restitution(7:50,2),ft);
...
clf(1);
    hold on
    plot(cjsr_ratio(:,1),cjsr_ratio(:,2),'ro',...
    ...
    xlim([0 1000]);
    ylim([-0.2 2]);
    title(sprintf('%.2f-%.2f*exp(-(x-%.2f)/%.2f), %.2f',...
        parameter(1),parameter(1),parameter(2),parameter(3),parameter(2)+parameter(3)));
