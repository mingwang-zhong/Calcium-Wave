...
#define stoptime (11501.0) //ms
#define PCL 1000.0	//ms, pacing cycle length
#define stopbeat 100000
...
int main(int argc, char **argv)
{
	...
	while ( step*DT < stoptime )
	{
		...
		///////////////////////////	Action Potential ///////////////////////////////////
		double time_s2 = atof(argv[10]);
		double stim = 0;
		if( (fmod(step*DT+PCL-time_before_beat,PCL) < 1.0 && step*DT > time_before_beat && step*DT < time_s2-400) || fmod(step*DT, time_s2) < 1.0 )
			stim = 80.0;
