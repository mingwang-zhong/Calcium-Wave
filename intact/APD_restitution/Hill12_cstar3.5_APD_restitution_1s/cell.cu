...
#define stoptime (12001.0) //ms
#define PCL 1000.0	//ms, pacing cycle length
#define stopbeat 10000
...
int main(int argc, char **argv)
{
	...
	while ( step*DT < stoptime )
	{
		...
		double stim = 0;
		if( (fmod(step*DT+PCL-time_before_beat,PCL) < 1.0 && step*DT > time_before_beat && step*DT < time_s2) || fmod(step*DT, time_s2) < 1.0 )
			stim = 80.0;
