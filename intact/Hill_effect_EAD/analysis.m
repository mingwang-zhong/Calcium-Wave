...
% H, latency, ci, cj, spark rate, Aci, Av, N_DAD, N_TA
statAll = zeros(num_H,32);
for h = 1:num_H
    ind = 1;
    H = Hill(h);
    % i, DAD or TA, t, ci, cj, spark rate, t_Aci, Aci, t_Av, Av
    result = zeros(5000,12); 
    ...
    for i = 1:Numi
        ...
        data = dlmread(sprintf('data/H%g_%g/wholecell.txt', H, i));
        indi = 1;
        ...
        for b=b0:maxbeat
            ...
            kCRUb = mean(data(forbase,40)); % baseline of diastolic rate
            ...
            % there are no DADs in this case, so we use the basal values
            if H~=6  % H<=2 || H>=10
                result(ind, 1) = i;
                result(ind, 2) = 0;
                result(ind, 3) = NaN;
    ...
    statAll(h,1) = H;
    statAll(h,2) = NumDAD; % Number of DAD
    statAll(h,3) = mean(resultDAD(:,11)); % latency
    statAll(h,4) = std(resultDAD(:,11));
    statAll(h,5) = mean(resultDAD(:,4)); % ci
    statAll(h,6) = std(resultDAD(:,4));
    statAll(h,7) = mean(resultDAD(:,5)); % cj
    statAll(h,8) = std(resultDAD(:,5));
    statAll(h,9) = mean(resultDAD(:,6)); % spark rate
    statAll(h,10) = std(resultDAD(:,6));
    statAll(h,11) = mean(resultDAD(:,8)); % max ci
    statAll(h,12) = std(resultDAD(:,8));
    statAll(h,13) = mean(resultDAD(:,10)); % max Vm
    statAll(h,14) = std(resultDAD(:,10));
    statAll(h,15) = mean(resultDAD(:,12)); % k_CRU
    statAll(h,16) = std(resultDAD(:,12));
    
    resultTA = result(result(:,2)>=1,:); % get the superthrethold DAD
    statAll(h,17) = numTA; % size(resultTA,1); % Number of TA
    statAll(h,18) = mean(resultTA(:,11)); % latency
    statAll(h,19) = std(resultTA(:,11));
    statAll(h,20) = mean(resultTA(:,4)); % ci
    statAll(h,21) = std(resultTA(:,4));
    statAll(h,22) = mean(resultTA(:,5)); % cj
    statAll(h,23) = std(resultTA(:,5));
    statAll(h,24) = mean(resultTA(:,6)); % spark rate
    statAll(h,25) = std(resultTA(:,6));
    statAll(h,26) = mean(resultTA(:,8)); % max ci
    statAll(h,27) = std(resultTA(:,8));
    statAll(h,28) = mean(resultTA(:,10)); % max Vm
    statAll(h,29) = std(resultTA(:,10));
    statAll(h,30) = mean(resultTA(:,12)); % k_CRU
    statAll(h,31) = std(resultTA(:,12));

    statAll(h,32) = num_beat_total;
    
    dlmwrite(sprintf('result_H%g.txt',H), result, 'delimiter', '\t');
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    str = {'Latency','Diastolic $c_i$','Diastolic $c_j$', ...
            'Spark rate', '$c_i$ amplitude', '$k_{CRU}$'};
    ind = [11 4 5 6 8 12];
    figure(2);
    clf(2);
        for i=1:6
            subplot(4,3,i);
            histogram(resultDAD(:,ind(i)));
            xlabel(str(i),'Interpreter','latex');
            title('Sub DAD');
        end

        for i=7:12
            subplot(4,3,i);
            histogram(resultTA(:,ind(i-6)));
            xlabel(str(i-6),'Interpreter','latex');
            title('TA');
        end
    ...
    saveas(gcf, sprintf('result_H%g.pdf',H), 'pdf');
    pause(2);
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
statAll(statAll(:,1)==0,:) = [];
dlmwrite('result_H_all.txt', statAll, 'delimiter', '\t');



str = {'Latency','Diastolic $c_i$','Diastolic $c_j$', ...
        'Spark rate', '$c_i$ amplitude', '$V_m$ amplitude', ...
        '$k_{CRU}$', 'Number of DAD/TA'};
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure(3); % DAD
clf(3);
    for i=1:8
        ...
        elseif i==7
            errorbar(statAll(2:end,1),statAll(2:end,15),statAll(2:end,16),'-o'); % k_CRU
        else
            errorbar(statAll(:,1),statAll(:,2*i+1),statAll(:,2*i+2),'-o');
        end
        xlabel('H');
        title(str(i),'Interpreter','latex');
    end
set(findobj('type','axes'),'FontSize',11);
set(gcf, 'PaperPosition', [-1 0 12 10]);
set(gcf, 'PaperSize', [10 10]);
saveas(gcf, 'H_DAD.pdf', 'pdf');
...
clf(4);
    for i=1:8
        subplot(3,3,i);
        if i==8
            plot(statAll(:,1), statAll(:,17),'-o');
        elseif i==7
            errorbar(statAll(:,1),statAll(:,30),statAll(:,31),'-o'); % k_CRU
        else
            errorbar(statAll(:,1),statAll(:,2*i+16),statAll(:,2*i+17),'-o');
        end
        xlabel('H');
        title(str(i),'Interpreter','latex');
    end
