			...
			for (k = 1; k < Nz-1; k++)
			{
				for (j = 1; j < Ny-1; j++)
				{
					...
					{	
						...
						double ku = alpha/( 1.0 + pow(custar/h_CRU2[ps].cp,Hill) ) + 2e-6;
						double kb = beta/( 1.0 + pow(cbstar/h_CRU2[ps].cp,Hill) ) + 2e-9; // /( 1.0 + pow(500/cjsr,4) )
						k_CRU += h_CRU2[ps].ncu*ku + h_CRU2[ps].ncb*kb;
